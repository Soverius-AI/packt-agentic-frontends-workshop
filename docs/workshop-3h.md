...
7. In Part 6, ask: **Show me when the Cooling room went into warning during the last seven days and when each warning ended.** Inspect the SQL generated for the single `query_historian` tool, the separate reviewer's semantic verdict, the deterministic policy result, and the generic result table. Previous days end at 14:00; today's warning is **Still active**.
