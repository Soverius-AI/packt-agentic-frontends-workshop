...
7. In Part 6, open the `historian-query` graph in Mastra Studio, then ask: **Show me when the Cooling room went into warning during the last seven days and when each warning ended.** Inspect the narrow `query_historian` call and the workflow's `generate-sql` → `review-sql` → `deterministic-validate-and-execute` steps, followed by the generic result table. Previous days end at 14:00; today's warning is **Still active**.
