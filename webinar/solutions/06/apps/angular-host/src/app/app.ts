...
import { alarmApprovalToolSchema, historianToolResultSchema } from '@packt-workshop/contracts';
import { AlarmApprovalEvents } from './alarm-approval-events';
import { FacilityApi } from './facility-api';
import { StreamingAutoScrollDirective } from './chat/streaming-auto-scroll.directive';
import { setViewInputSchema, updateFiltersInputSchema } from './frontend-tool-schemas';
import { z } from 'zod';
...
@Component({
  selector: 'app-root',
  imports: [CopilotA2UIActivityRenderer, CopilotChat, StreamingAutoScrollDirective],
...
export class App {
  ...
  protected setupTools() {
    registerHumanInTheLoop({
      name: 'raise_alarm',
      description: 'Ask the user if they want to raise an alarm, use the metric returned by list_metrics',
    ...
    registerFrontendTool({
      ...
      agentId: 'default',
      description: 'List available shift managers',
      parameters: z.object({}),
      handler: async () => {
        return this.shiftManagerOptions();
      },
    });
    ...
    registerFrontendTool({
      ...
      parameters: z.object({}),
      handler: async () => this.rooms().flatMap(room => room.metrics.map(({ id, name }) => ({ id, name, room: room.name })))
    })

    registerFrontendTool({
      name: 'list_rooms',
      agentId: 'default',
      description: 'Shows available rooms',
      parameters: z.object({}),
      handler: async () => {
        return this.rooms().map(({ id, name }) => ({ id, name }));
      },
    });

    registerFrontendTool({
      name: 'set_view',
      description: 'switch between snapshot and reading-log. Preserve filters',
      agentId: 'default',
      parameters: setViewInputSchema,
      handler: async (input) => {
        this.setDisplayMode(input.view);
        return { view: this.displayMode() };
      },
    });
    ...
    registerFrontendTool({
      ...
      agentId: 'default',
      description:
        'update the supplied filters using the available options. Omitted filters keep their values. Null clears that filter',
      parameters: updateFiltersInputSchema,
      handler: (filterData) => this.updateFilters(filterData),
    });
  }
  ...
  protected async updateFilters(filters: {
    ...
    if (filters.roomId && !this.rooms().some((room) => room.id === filters.roomId))
      return { ok: false, error: 'Unknown room ID. Use list_rooms to find a valid ID.' };
    if (filters.metricId && !this.metricOptions().some((metric) => metric.id === filters.metricId))
      return {
        ok: false,
        error: 'Unknown metric ID. Ask the operator for an exact supported metric ID.',
      };
    if (filters.shiftManager && !this.shiftManagerOptions().includes(filters.shiftManager))
      return {
        ok: false,
        error: 'Unknown shift manager. Use list_shift_managers to find a valid name.',
      };
    if (
      filters.condition &&
