...
**Status:** Prepared walkthrough; presenter implementation and recovery snapshot pending.
**Start checkpoint:** webinar-07. **Working branch:** webinar-08.
...
3. In agent.ts select the prepared webinar-08 prompt. It preserves the operator's
...
5. In create-copilot-runtime.ts use the prepared HistorianBridge in place of
   ...
   requests. Reuse it rather than implementing compatibility code live.
...
The completed 08 checkpoint is not registered yet. After the live implementation
and checks, update manifest.json, solutions, demo prompts and presenter tests and
save webinar-08. Until then the presenter desk and selector expose 01–07 only.
Selecting a checkpoint never resets stored readings or alarms. Restart the backend
when moving between the fixed-reading and dataset implementations.
